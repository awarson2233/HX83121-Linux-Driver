...
static int himax_mcu_register_read(uint8_t *addr, uint8_t *buf, uint32_t len)
{
	...
	ret = himax_bus_write(pic_op->addr_ahb_addr_byte_0[0] /*0x00*/, addr, NULL,
		4);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
	...
	ret = himax_bus_write(pic_op->addr_ahb_access_direction[0] /*0x0C*/, NULL,
	...
	ret = himax_bus_read(pic_op->addr_ahb_rdata_byte_0[0] /*0x08*/, buf, len);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		mutex_unlock(&private_ts->reg_lock);
		return BUS_FAIL;
...
static void himax_mcu_interface_on(void)
{
	...
	ret = himax_bus_read(pic_op->addr_ahb_rdata_byte_0[0] /*0x08*/, tmp_data,
		DATA_LEN_4);
	if (ret < 0) {
		E("%s: bus access fail!\n", __func__);
		return;
