...
/* Map function pointers to implementations (incell) */
struct himax_core_fp {
/* CORE_IC */
	void (*fp_burst_enable)(uint8_t auto_add_4_byte);                 // -> himax_mcu_burst_enable
	int (*fp_register_read)(uint8_t *addr, uint8_t *buf, uint32_t len); // -> himax_mcu_register_read
	int (*fp_register_write)(uint8_t *addr, uint8_t *val, uint32_t len); // -> himax_mcu_register_write
	void (*fp_interface_on)(void);                                     // -> himax_mcu_interface_on
	void (*fp_sense_on)(uint8_t FlashMode);                            // -> hx83121a_sense_on
	void (*fp_tcon_on)(void);                                          // -> NULL
	bool (*fp_watch_dog_off)(void);                                    // -> NULL
	bool (*fp_sense_off)(bool check_en);                               // -> himax_mcu_sense_off
	void (*fp_sleep_in)(void);                                         // -> NULL
	bool (*fp_wait_wip)(int Timing);                                   // -> himax_mcu_wait_wip
	void (*fp_init_psl)(void);                                         // -> himax_mcu_init_psl
	void (*fp_resume_ic_action)(void);                                 // -> himax_mcu_resume_ic_action
	void (*fp_suspend_ic_action)(void);                                // -> himax_mcu_suspend_ic_action
	void (*fp_power_on_init)(void);                                    // -> himax_mcu_power_on_init
	bool (*fp_slave_tcon_reset)(void);                                 // -> NULL
	bool (*fp_slave_adc_reset_slave)(void);                            // -> NULL
	bool (*fp_slave_wdt_off_slave)(void);                              // -> NULL
/* CORE_IC */
...
/* CORE_FW */
	void (*fp_parse_raw_data)(struct himax_report_data *hx_touch_data,
			...
			uint8_t diag_cmd,
			int16_t *mutual_data,
			int16_t *self_data);                                          // -> diag_mcu_parse_raw_data (via wrapper)
	void (*fp_system_reset)(void);                                     // -> himax_mcu_system_reset
	int (*fp_Calculate_CRC_with_AP)(unsigned char *FW_content,
			int CRC_from_FW,
			int len);                                                     // -> himax_mcu_Calculate_CRC_with_AP
	uint32_t (*fp_check_CRC)(uint8_t *start_addr, int reload_length);  // -> himax_mcu_check_CRC
	void (*fp_set_reload_cmd)(uint8_t *write_data,
			int idx,
			uint32_t cmd_from,
			uint32_t cmd_to,
			uint32_t cmd_beat);                                           // -> himax_mcu_set_reload_cmd
	bool (*fp_program_reload)(void);                                   // -> himax_mcu_program_reload
	void (*fp_set_SMWP_enable)(uint8_t SMWP_enable, bool suspended);   // -> himax_mcu_set_SMWP_enable
	void (*fp_set_HSEN_enable)(uint8_t HSEN_enable, bool suspended);   // -> himax_mcu_set_HSEN_enable
	void (*fp_diag_register_set)(uint8_t diag_command,
			uint8_t storage_type, bool is_dirly);                         // -> himax_mcu_diag_register_set
	void (*_clr_fw_reord_dd_sts)(void);                                // -> hx_clr_fw_reord_dd_sts
	void (*_ap_notify_fw_sus)(int suspend);                            // -> hx_ap_notify_fw_sus
#if defined(HX_TP_SELF_TEST_DRIVER)
	void (*fp_control_reK)(bool enable);                               // -> (not set here)
#endif
	int (*fp_chip_self_test)(struct seq_file *s, void *v);             // -> himax_mcu_chip_self_test
	void (*fp_idle_mode)(int disable);                                 // -> himax_mcu_idle_mode
	void (*fp_reload_disable)(int disable);                            // -> himax_mcu_reload_disable (or hx_dis_rload_0f under HX_ZERO_FLASH)
	int (*fp_read_ic_trigger_type)(void);                              // -> himax_mcu_read_ic_trigger_type
	int (*fp_read_i2c_status)(void);                                   // -> himax_mcu_read_i2c_status
	void (*fp_read_FW_ver)(void);                                      // -> himax_mcu_read_FW_ver
	bool (*fp_read_event_stack)(uint8_t *buf, uint8_t length);         // -> himax_mcu_read_event_stack
	void (*fp_return_event_stack)(void);                               // -> himax_mcu_return_event_stack
	bool (*fp_calculateChecksum)(bool change_iref, uint32_t size);     // -> himax_mcu_calculateChecksum
	void (*fp_read_FW_status)(void);                                   // -> himax_mcu_read_FW_status
	void (*fp_irq_switch)(int switch_on);                              // -> himax_mcu_irq_switch
	int (*fp_assign_sorting_mode)(uint8_t *tmp_data);                  // -> himax_mcu_assign_sorting_mode
	int (*fp_check_sorting_mode)(uint8_t *tmp_data);                   // -> himax_mcu_check_sorting_mode
	int (*fp_get_max_dc)(void);                                        // -> NULL
	uint8_t (*fp_read_DD_status)(uint8_t *cmd_set, uint8_t *tmp_data); // -> himax_mcu_read_DD_status
	int (*fp_ulpm_in)(void);                                           // -> himax_mcu_ulpm_in (if HX_ULTRA_LOW_POWER)
	int (*fp_black_gest_ctrl)(bool enable);                            // -> himax_mcu_black_gest_ctrl (if HX_ULTRA_LOW_POWER)
	int	(*_diff_overlay_bin)(void);                                   // -> (not set here)
/* CORE_FW */

/* CORE_FLASH */
	void (*fp_chip_erase)(void);                                       // -> himax_mcu_chip_erase
	bool (*fp_block_erase)(int start_addr, int length);                // -> himax_mcu_block_erase
	bool (*fp_sector_erase)(int start_addr);                           // -> himax_mcu_sector_erase
	void (*fp_flash_programming)(uint8_t *FW_content, int FW_Size);    // -> himax_mcu_flash_programming
	void (*fp_flash_page_write)(uint8_t *write_addr, int length,
			uint8_t *write_data);                                         // -> himax_mcu_flash_page_write
	int (*fp_fts_ctpm_fw_upgrade_with_sys_fs_32k)(unsigned char *fw,
			int len, bool change_iref);                                   // -> himax_mcu_fts_ctpm_fw_upgrade_with_sys_fs_32k
	int (*fp_fts_ctpm_fw_upgrade_with_sys_fs_60k)(unsigned char *fw,
			int len, bool change_iref);                                   // -> himax_mcu_fts_ctpm_fw_upgrade_with_sys_fs_60k
	int (*fp_fts_ctpm_fw_upgrade_with_sys_fs_64k)(unsigned char *fw,
			int len, bool change_iref);                                   // -> himax_mcu_fts_ctpm_fw_upgrade_with_sys_fs_64k
	int (*fp_fts_ctpm_fw_upgrade_with_sys_fs_124k)(unsigned char *fw,
			int len, bool change_iref);                                   // -> himax_mcu_fts_ctpm_fw_upgrade_with_sys_fs_124k
	int (*fp_fts_ctpm_fw_upgrade_with_sys_fs_128k)(unsigned char *fw,
			int len, bool change_iref);                                   // -> himax_mcu_fts_ctpm_fw_upgrade_with_sys_fs_128k
	int (*fp_fts_ctpm_fw_upgrade_with_sys_fs_255k)(unsigned char *fw,
			int len, bool change_iref);                                   // -> himax_mcu_fts_ctpm_fw_upgrade_with_sys_fs_255k
	void (*fp_flash_dump_func)(uint8_t local_flash_command,
			int Flash_Size, uint8_t *flash_buffer);                       // -> himax_mcu_flash_dump_func
	bool (*fp_flash_lastdata_check)(uint32_t size);                     // -> himax_mcu_flash_lastdata_check
	bool (*fp_bin_desc_get)(unsigned char *fw, uint32_t max_sz);        // -> hx_mcu_bin_desc_get
	bool (*fp_ahb_squit)(void);                                        // -> NULL
	void (*fp_flash_read)(uint8_t *r_data, int start_addr, int length); // -> NULL
	bool (*fp_sfr_rw)(uint8_t *addr, int length,
			uint8_t *data, uint8_t rw_ctrl);                              // -> NULL
	bool (*fp_lock_flash)(void);                                       // -> NULL
	bool (*fp_unlock_flash)(void);                                     // -> NULL
	void (*fp_init_auto_func)(void);                                   // -> NULL
	int (*_diff_overlay_flash)(void);                                  // -> hx_mcu_diff_overlay_flash
/* CORE_FLASH */

/* CORE_SRAM */
	void (*fp_sram_write)(uint8_t *FW_content);                         // -> himax_mcu_sram_write
	bool (*fp_sram_verify)(uint8_t *FW_File, int FW_Size);              // -> himax_mcu_sram_verify
	bool (*fp_get_DSRAM_data)(uint8_t *info_data, bool DSRAM_Flag);     // -> himax_mcu_get_DSRAM_data
/* CORE_SRAM */

/* CORE_DRIVER */
	void (*fp_chip_init)(void);                                         // -> himax_mcu_init_ic
	void (*fp_pin_reset)(void);                                         // -> himax_mcu_pin_reset (if HX_RST_PIN_FUNC)
	uint8_t (*fp_tp_info_check)(void);                                  // -> himax_mcu_tp_info_check
	void (*fp_touch_information)(void);                                 // -> himax_mcu_touch_information
	void (*fp_calc_touch_data_size)(void);                              // -> himax_mcu_calcTouchDataSize
	void (*fp_reload_config)(void);                                     // -> NULL
	int (*fp_get_touch_data_size)(void);                                // -> himax_mcu_get_touch_data_size
	void (*fp_usb_detect_set)(uint8_t *cable_config);                    // -> himax_mcu_usb_detect_set
	int (*fp_hand_shaking)(void);                                       // -> himax_mcu_hand_shaking
	int (*fp_determin_diag_rawdata)(int diag_command);                  // -> himax_mcu_determin_diag_rawdata
	int (*fp_determin_diag_storage)(int diag_command);                  // -> himax_mcu_determin_diag_storage
	int (*fp_cal_data_len)(int raw_cnt_rmd, int HX_MAX_PT, int raw_cnt_max); // -> himax_mcu_cal_data_len
	bool (*fp_diag_check_sum)(struct himax_report_data *hx_touch_data);  // -> himax_mcu_diag_check_sum
	void (*fp_diag_parse_raw_data)(struct himax_report_data *hx_touch_data,
			int mul_num,
			int self_num,
			uint8_t diag_cmd,
			int32_t *mutual_data,
			int32_t *self_data);                                           // -> himax_mcu_diag_parse_raw_data
	void (*fp_ic_reset)(uint8_t loadconfig, uint8_t int_off);           // -> himax_mcu_ic_reset (if HX_RST_PIN_FUNC)
	int (*fp_ic_excp_recovery)(uint32_t hx_excp_event,
			uint32_t hx_zero_event, uint32_t length);                     // -> himax_mcu_ic_excp_recovery (if HX_EXCP_RECOVERY)
	void (*fp_excp_ic_reset)(void);                                     // -> himax_mcu_excp_ic_reset (if HX_EXCP_RECOVERY)
	void (*fp_resend_cmd_func)(bool suspended);                         // -> himax_mcu_resend_cmd_func
#if defined(HX_TP_PROC_GUEST_INFO)
	int (*guest_info_get_status)(void);                                 // -> himax_guest_info_get_status
	int (*read_guest_info)(void);                                       // -> hx_read_guest_info
#endif
/* CORE_DRIVER */
	int (*fp_turn_on_mp_func)(int on);                                  // -> hx_turn_on_mp_func
#if defined(HX_ZERO_FLASH)
	void (*fp_clean_sram_0f)(uint8_t *addr, int write_len, int type);   // -> himax_mcu_clean_sram_0f
	void (*fp_write_sram_0f)(uint8_t *addr, const uint8_t *data,
		uint32_t len);                                                   // -> himax_mcu_write_sram_0f
	int (*fp_write_sram_0f_crc)(uint8_t *addr, const uint8_t *data,
		uint32_t len);                                                   // -> himax_sram_write_crc_check
	int (*fp_firmware_update_0f)(const struct firmware *fw_entry, int type); // -> himax_mcu_firmware_update_0f
	int (*fp_0f_op_file_dirly)(char *file_name);                        // -> hx_0f_op_file_dirly
	int (*fp_0f_excp_check)(void);                                      // -> himax_mcu_0f_excp_check
	void (*fp_0f_reload_to_active)(void);                               // -> (not set here)
#if defined(HX_0F_DEBUG)
	void (*fp_read_sram_0f)(const struct firmware *fw_entry,
			uint8_t *addr,
			int start_index,
			int read_len);                                                // -> himax_mcu_read_sram_0f
	void (*fp_read_all_sram)(uint8_t *addr, int read_len);              // -> himax_mcu_read_all_sram
	void (*fp_firmware_read_0f)(const struct firmware *fw_entry, int type); // -> himax_mcu_firmware_read_0f
#endif
#endif
	int (*fp_0f_overlay)(int ovl_type, int mode);                       // -> (not set here)
	void (*fp_suspend_proc)(bool suspended);                             // -> himax_suspend_proc
	void (*fp_resume_proc)(bool suspended);                              // -> himax_resume_proc

#if defined(XM_GAME_MODE)
	int (*fp_game_mode_set)(uint8_t *touchfeature);                      // -> himax_touchfeature_set
	int (*fp_Screen_orientation_set)(uint8_t direction);                 // -> himax_Screen_orientation_set
#endif

#if defined(HX_PEN_DETECT_GLOBAL)
	int (*fp_pen_charge_mode_set)(int enable);                           // -> himax_pen_charge_detect_func
#endif
};
